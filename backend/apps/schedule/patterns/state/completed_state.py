...
class CompletedState(ScheduleState):
    ...
    def handle_cancel(self, event):
        raise InvalidStateTransitionError('Jadwal sudah selesai, tidak bisa dibatalkan')

    def handle_complete(self, event):
        raise InvalidStateTransitionError('Jadwal sudah selesai')

    def handle_reschedule(self, event, new_date, new_time):
        raise InvalidStateTransitionError('Jadwal sudah selesai, tidak bisa dijadwalkan ulang')
