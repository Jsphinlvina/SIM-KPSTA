...
class GuidanceScheduleManager:
    ...
    def start_schedule(self, event):
        get_state(event.status).handle_schedule(event)
        event.save()
        return event

    def complete_schedule(self, event):
        get_state(event.status).handle_complete(event)
        event.save()
        return event

    def cancel_schedule(self, event):
        get_state(event.status).handle_cancel(event)
        event.save()
    ...
    def reschedule(self, event, new_date, new_time):
        ...
        if not detector.is_available(event.lecturer_id, new_date, new_time, exclude_event_id=event.id):
            raise ScheduleConflictError('Dosen tidak tersedia pada tanggal dan waktu baru tersebut')

        old_date = event.date
        get_state(event.status).handle_reschedule(event, new_date, new_time)
        event.save()
        detector.invalidate(event.lecturer_id, old_date)
        detector.invalidate(event.lecturer_id, new_date)
        return event
