...
export default function DetailMahasiswa({
  ...
  const targetDosen = manager.getDistribusiData().find(
    (d) =>
      d.nama.toLowerCase().includes(namaDosen.toLowerCase()) ||
  ...
  return (
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
      <div className="w-[900px] bg-white rounded-3xl shadow-xl overflow-hidden">
        {/* Header */}
        <div className="flex justify-between items-center px-8 py-5 border-b">
          <div>
            <h2 className="text-2xl font-bold text-[#355872]">Mahasiswa Bimbingan</h2>
            <p className="text-gray-500 mt-1">{namaDosen}</p>
          </div>
          <button
            onClick={onClose}
            className="p-2 rounded-xl hover:bg-gray-100 text-[#355872] cursor-pointer"
          >
            <X size={22} />
          </button>
        </div>
        ...
        <div className="max-h-[500px] overflow-auto">
          <div className="grid grid-cols-[60px_180px_1fr_1.5fr] bg-[#EAF4FB] px-8 py-4 font-semibold text-[#355872]">
            <div>No</div>
            <div>NIM / NRP</div>
          ...
          {mahasiswaList.map((mhs, index) => (
            <div
              key={index}
              className="grid grid-cols-[60px_180px_1fr_1.5fr] px-8 py-5 border-b border-[#eef4f8] text-[#355872] items-center"
            >
              <div>{index + 1}</div>
              <div>{mhs.nim || "-"}</div>
              <div className="font-bold">{mhs.nama}</div>
              <div className="text-sm text-gray-500 font-medium">{mhs.topik || "-"}</div>
            </div>
          ))}
