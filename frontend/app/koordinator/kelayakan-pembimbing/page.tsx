...
export default function KelayakanPembimbingKoordinatorPage() {
  ...
  const handleApprove = async (id: number) => {
    ...
    try {
      ...
      showToast("Kelayakan disetujui, dilanjutkan ke tahap berikutnya.", true);
    } catch (err: any) {
  ...
  const handleReject = async () => {
    ...
    try {
      await api.post(`/bimbingan/${rejectId}/check-approval/`, {
        reject: true,
        catatan: catatan || "Ditolak oleh koordinator.",
      });
      ...
      <div className="flex-1 p-10">
        ...
        <div className="flex items-start gap-4 mb-10">
          ...
          <div>
            ...
            <p className="text-gray-500 mt-2">Verifikasi kelayakan dosen pembimbing yang diusulkan</p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-3xl border border-[#e6eef5] shadow-sm overflow-hidden">
            <div className="grid grid-cols-[60px_1fr_1fr_1fr_180px] bg-[#EAF4FB] px-8 py-5 font-semibold text-[#355872]">
              <div>No</div>
              <div>Mahasiswa</div>
              <div>Topik</div>
            ...
            {list.map((item, idx) => (
              <div
                key={item.proses_id}
                className="grid grid-cols-[60px_1fr_1fr_1fr_180px] items-center px-8 py-6 border-t border-[#eef4f8]"
              >
